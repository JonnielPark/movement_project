...
현재 p01 squat gate다. `feature_depth_gravity = 0.0`인 동안 `corrected_3d_hypothesis` row는
review 후보일 뿐이다.
...
| `control.stability.hip_center_z_std` | `dual_domain_compare` | 현재 model-depth axis와 향후 corrected candidate comparison | depth evidence는 low-confidence; scoring 전 norm-vs-corrected sensitivity 필요 | corrected depth score-excluded | torso-length ratio | `feature_depth_gravity = 0.0`인 동안 corrected depth는 score에 쓰지 않는다. Sensitivity와 availability만 보고한다. |
...
| `control.compensation.pelvic_rotation` | `corrected_3d_hypothesis` | left/right hip model-depth asymmetry와 향후 corrected candidate | depth-sensitive; correction burden, residual, sensitivity report 필요 | depth gravity가 0인 동안 score-excluded | torso-length ratio hip-depth asymmetry | 현재 public path에서는 score하지 않는다. Low-confidence corrected-3D review로만 사용한다. |
...
| `biomech.com.range_z` | `dual_domain_compare` | model-depth CoM proxy와 향후 corrected candidate comparison | depth-sensitive; corrected 사용 전 burden report 필요 | corrected depth score-excluded | torso-length ratio | Multi-video sensitivity 근거 전까지 provenance 또는 low-confidence로 둔다. |
