...
2. 목표 반복 수는 10회이다. 10회를 채우기 어려운 운동은 자세가 완전히 무너지기 전의 최대
   반복 수를 기록하고, 실제 반복 수를 annotation 또는 recording metadata에 남긴다.
3. 팔 움직임이 분석 대상이 아닌 운동에서는 양손을 고정하여 팔 반동이 관절 궤적에 섞이지
   않도록 한다.
4. 아래의 "분석을 방해하는 수행 패턴"은 자동 제외 규칙이 아니다. 데이터 품질 경고,
...
기준 매트 앞 모서리를 대각선 약 45도로 바라보는 위치에 카메라를 둔다. 렌즈 높이는
...
기준 매트 긴 변을 정면으로 마주 보는 완전한 측면 약 90도 위치에 카메라를 둔다. 렌즈 높이는
...
기준 매트 옆면을 마주 보는 완전한 측면 약 90도 위치에 카메라를 둔다. 카메라는 바닥과
가까운 0-30 cm 높이에 둔다.
...
3. 목표는 10회이다. 난이도가 높아 10회를 채우기 어렵다면 무리하지 말고 자세가 완전히
   무너지기 전의 최대 반복 수까지만 수행한다. 필요하면 같은 방식으로 3세트까지 촬영한다.
...
기준 매트 앞 모서리를 대각선 약 45도로 바라보는 위치에 카메라를 둔다. 카메라는 바닥과
가까운 0-30 cm 높이에 둔다.
