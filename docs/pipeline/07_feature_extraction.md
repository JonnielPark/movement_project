# 07. 피처 추출 (Feature Extraction)

**문서 버전:** 1.4.1
**최종 갱신:** 2026-07-09
**영문 동기화:** `docs_eng/pipeline/07_feature_extraction.md`는 동일 버전의 영문 번역본이다.

파이프라인 단계 ⑦은 정규화된 pose 데이터에서 movement-quality feature를 계산한다. Pose 좌표는
수정하지 않는다. 모든 출력은 numeric value, unit과 ⑨ Biomarker Derivation에 필요한 명시적
운영 메타데이터, 즉 availability, reliability, evidence axes, coordinate reference,
feature family, focus tier를 가진 `FeatureRecord`다. `source_fields`는 debug/report export용
선택적 audit reference이며 scoring 필수 입력이 아니다.
...
recording metadata          camera_zone, camera_height_level이 있으면 사용
...
source_fields 문자열에 의존하는 scoring logic
...
control.*
    ...
    excessive_trunk_flexion, heel_lift, pelvis_rotation 같은 compensation pattern.
```
...
`availability`, reliability metadata를 붙여 방출하고, ⑨가 scoring weight를 결정한다. 이렇게 하면
...
downstream으로 전달해야 한다. 이 필드들은 feature value나 scoring weight를 직접 바꾸지 않는다.
대신 ⑨와 notebook에서 좌표 기준, evidence 경로, scoring 맥락을 명시적으로 확인할 수 있게 한다.
...
계획된 해석 패턴만 유지한다. 예를 들어 knee profile은 knee range of motion, knee movement path,
knee role alignment,
...
shoulder             shoulder flexion/extension과 scapular stability proxy를 유지한다.
                     ...
                     rotation은 추가하지 않는다.
elbow                elbow flexion/extension과 계획된 elbow-tracking 패턴만 유지하고,
...
높은 `depth_dependency`를 함께 가지며, 향후 corrected analysis evidence와 비교할 의도가 있을
...
spatial.range_of_motion.xy.<joint_angle>
    ...
    우선 점수화 가능 feature로 사용한다.

spatial.range_of_motion.xyz.<joint_angle>
    Normalized x/y와 model/analysis z를 함께 쓰는 mixed-axis included-angle range of motion.
...
spatial.movement_path.arc_length_xy.<landmark>
    ...
    보통 diagnostic 또는 낮은 weight evidence이지, foot/hand가 실제로 움직였다는
    직접 증거가 아니다.
...
spatial.movement_path.arc_length_xy.<landmark>
    ...
    evidence로 답해질 수 있고 recording view가 baseline과 호환될 때 우선 점수화 가능 feature가 된다.

spatial.movement_path.arc_length_xyz.<landmark>
    Normalized x/y와 model/analysis z를 함께 쓰는 mixed-axis movement path다. 이는
    depth-sensitive evidence이며 review와 analysis-3D 비교에는 유용하지만, 현재 monocular
    pipeline에서는 보통 `xy` variant보다 낮은 scoring weight를 가져야 한다.
...
composite scoring
    ...
    `xyz`는 낮은 weight의 depth-sensitive evidence 또는 corrected-3D-hypothesis provenance로
...
control.compensation.pelvis_rotation.xyz
    ...
    지배할 수 있으므로 view/analysis-evidence policy가 승격하기 전까지 low-weight 또는
    report-only로 둔다.
```
...
재사용하지 말고, 별도 feature id와 낮은 weight로 방출해야 한다.

좌표에서 파생되는 control record도 명시적인 evidence variant를 가진다. Recording-view analysis evidence는
feature id에 `xy` 토큰을 사용하고, depth가 섞인 analysis evidence는 `xyz` 토큰을 사용한다. `xyz`
analysis evidence는 monocular model-depth reliability가 낮으면 낮은 weight로 점수화하거나 withheld해야 한다.
...
control.compensation.knee_varus.xy.<side>
    ...
    analysis evidence다. raw monocular depth에 기대지 않고 corrected/body-frontal plane에서 medial/lateral
...
control.compensation.excessive_trunk_flexion.xy
    ...
    해석을 뒷받침할 때 선호하는 public score-eligible feature다.
...
control.compensation.excessive_trunk_flexion.xyz
    ...
    현재 monocular pipeline에서는 comparative evidence로만 쓰며 reduced weight를 가져야 한다.
...
control.compensation.pelvis_rotation.xyz
    Left-right hip model-depth asymmetry에 기반한 depth-sensitive proxy다. View/analysis evidence가
...
source_fields         선택적 phase_segmentation audit reference 포함
...
camera-zone, 운영 메타데이터를 보존한다. Audit reference는 debug/report export가 필요할 때
선택적으로 포함할 수 있다.
...
data/processed/features/<recording_id>_features.csv
    ...
    exercise_id, rep_id, phase, value, unit, availability,
    ...
    feature_family, camera_zone, role_context다. `source_fields`는 선택적
    audit reference로 포함할 수 있지만 필수 payload column은 아니다.
...
data/processed/features/<recording_id>_feature_qc.json
    ...
    feature-family count, phase count, debug export에 포함된 경우 audit-reference
    coverage 등을 담는다.
```

CSV round-trip은 row count와 필수 운영 column을 보존해야 한다.
`availability_reasons`, `landmark_ids`, `role_context`, 선택적 `source_fields` 같은 구조화 field는
CSV 저장을 위해 직렬화할 수 있지만, 이후 code path의 canonical object contract는 in-memory
record다.
...
Feature registry coverage
    YAML feature_domain entry와 compensation pattern가 implemented, routed to another
...
- ⑪ Simulation은 pose-detectable audit entry를 perturbation input로 사용할 수 있다.
...
- source fields, confidence policy, test가 준비된 compensation rule 추가.
