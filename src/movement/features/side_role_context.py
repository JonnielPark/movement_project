...
_LEGACY_ATTRIBUTION_COLUMN_ALIASES = {
    "side_role_consistent": "attribution_consistent",
    "side_role_confidence": "attribution_confidence",
    "side_role_action": "attribution_action",
}
...
@dataclass
class SideRoleContextThresholds:
    """Decision thresholds (τ) for active-side role context.
...
def resolve_side_role_context(
    df: pd.DataFrame,
    exercise_definition: "ExerciseDefinition",
    thresholds: SideRoleContextThresholds | None = None,
    ...
    thresholds : SideRoleContextThresholds, optional
    ...
    -------
    df : pd.DataFrame
        Dataframe with side-role context columns added.
    report : SideRoleContextReport
    """
    if thresholds is None:
        thresholds = SideRoleContextThresholds()

    df = df.copy()
    report = SideRoleContextReport(mode=mode)
    ...
    for rep_num_zero, rep_id in enumerate(rep_ids_sorted):
        ...
        df.loc[row_mask, "side_role_consistent"] = consistent
        df.loc[row_mask, "side_role_confidence"] = confidence
        df.loc[row_mask, "side_role_action"] = action
...
def attribute_motion(*args: Any, **kwargs: Any) -> tuple[pd.DataFrame, SideRoleContextReport]:
    ...
    for canonical, legacy in _LEGACY_ATTRIBUTION_COLUMN_ALIASES.items():
        df[legacy] = df[canonical] if canonical in df.columns else None
    return df, report


# Backward-compatible aliases. New code should import the SideRoleContext* names.
AttributionThresholds = SideRoleContextThresholds
AttributionReport = SideRoleContextReport
