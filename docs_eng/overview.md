...
compensatory movement patterns at the rep and phase levels. This converts
...
hand-equipment contact, and resistance direction. Highly dynamic or spatially
traveling tasks such as jumping, running, or change-of-direction movements would
require ground-contact events, flight phases, global travel paths, tracking
...
exercise-definition system uses split YAML artifacts: the exercise definition keeps
...
Longer movement sequences are represented by `ExerciseSessionDefinition` artifacts
under `data/definitions/exercise_sessions/`. A session definition orders existing
exercise blocks and gives them one uniform rest policy between blocks; it does
not create a separate general/mixed exercise type.
...
compensation_patterns  movement patterns to monitor
feature_domains       which spatial / temporal / control features to activate
biomechanical_focus   which proxy metrics to compute
quality_rules         confidence threshold, max interpolation gap, …

data/protocols/performance/<exercise_id>.yaml
performance_protocol  planned count and side-sequence rules
...
blocks                ordered references to existing exercise_id values
rest_policy           uniform planned rest between consecutive blocks
```
...
Input
    ...
    Exercise YAML      exercise definition; optional exercise session definition
...
Steps
    ...
    ⑤  Normalization        hip-center translation + median torso-length scale
        ⑤-1 optional canonicalization filters, configured under normalization
...
Output
    ...
    Biomechanical proxy table — BiomechRecord list, rep-level, confidence-weighted
    ...
    Canonicalization report — optional ⑤-1 coordinate-alignment priors, correction magnitude, confidence notes
...
| ② Annotation | Pose DataFrame, Annotation CSV, optional recording metadata | Merges manual annotation information at frame level and constructs `exercise_id`, `execution_pattern`, `starting_side`, the initial `phase`, filming metadata, and performance/failure audit summaries. | Annotated DataFrame, annotation report |
...
| ⑨ Biomarker Derivation | FeatureRecord, BiomechRecord, baseline | Converts individual metrics into BiomarkerRecord entries and derives Z-score-based domain scores and composite scores. Coordinate-correction magnitude and observation quality remain explicit data-confidence fields, while trace provenance stays in stage/audit reports. | BiomarkerRecord, BiomarkerScoreRecord, InterpretationRecord |
...
```text
Spatial            range of motion, role alignment, movement path, compensation patterns
...
quality. A low-confidence view, low confidence, or model-depth limitation should
surface as explicit availability/confidence fields, withholding logic, and
sidecar audit reports rather than an automatic movement-quality penalty.
...
- Optional canonicalization is configured under `normalization.canonicalization`.
  It preserves raw/norm coordinates and emits separate analysis-space coordinates plus
  confidence/correction reports. It must not fit the pose to a good-movement
  template or erase true compensation patterns. See
  [05_normalization.md](pipeline/05_normalization.md) and the detailed reference
  in [05_1_canonicalization.md](pipeline/05_1_canonicalization.md).
...
Implemented
    ...
    normalization with optional canonicalization analysis-evidence reports, segmentation, feature-side-role context, features, biomech proxies,
    biomarker scoring, interpretation rules, and synthetic-normal baseline.

Review-only / disabled by default
    optional ⑤-1 canonicalization priors, including support-plane, movement-plane, and
