...
- Per-segment subject-fitted segment-length estimation (replace constant trunk proxy)
- Inverted-posture handling for `pike_pushup` and `plank_shoulder_tap`
  (anthropometry tables are body-orientation invariant, but landmark mapping
  needs verification under inverted closed-chain postures)
- Robustness verification of CoM stability under occlusion / noise (Task ⑫)
