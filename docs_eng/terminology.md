...
| Phase | Meaningful sub-interval within one rep. Two distinct labeling schemes coexist: (1) **kinetic phase** labels (eccentric / isometric / concentric) stored in `phase_model.expected_ratio` for duration-ratio reference; (2) **kinematic phase** labels (Descent / Ascent / Bottom_Hold etc.) written to the `phase` column by ⑥ Segmentation. These are deliberately decoupled. |
| Kinematic phase | A trajectory-based sub-interval of one rep defined by the movement direction of a reference landmark (e.g., hip-center descent vs. ascent). Labels: `Descent`, `Ascent`, `Bottom_Hold` (resistance exercises); `Lift`, `Tap`, `Return` (task exercises). Written to the `phase` column by ⑥ Segmentation; never mixed with kinetic terms (eccentric, concentric). |
| Rep segmentation | Semi-automatic procedure that determines repetition start/end boundaries from a normalized reference-landmark trajectory and confirms `rep_id`. Reads the new `rep_segmentation` YAML block in the exercise definition. |
| Phase segmentation | Procedure that splits kinematic phase boundaries inside each confirmed rep and fills the `phase` column. Keeps the existing code identifier and exercise-YAML key `phase_segmentation`. |
...
| Segmentation failure point | A frame or frame interval where ⑥ Segmentation cannot reliably decide a rep boundary or phase boundary. Causes may include poor pose quality, insufficient ROM, multiple candidates, reference-landmark occlusion, or the need for manual correction. Failure points are recorded in the report rather than hidden; rep-level or phase-level metrics are not emitted for the affected range until it is resolved. |
...
| Rep segmentation block | The `rep_segmentation:` YAML block in an exercise definition that declares the reference landmark, reference axis, boundary-detection logic, and smoothing parameters for repetition-boundary detection in ⑥ Segmentation. |
| Phase segmentation block | The existing `phase_segmentation:` YAML block in an exercise definition that declares the intra-rep phase sequence, reference landmark, reference axis, and inflection-detection logic. Absent in `generic.yaml`; when absent, phase splitting no-ops. |
...
| Segmentation | Tracks joint motion to confirm rep boundaries through `rep_segmentation`, then uses the existing `phase_segmentation` process to write intra-rep kinematic phase labels (Descent / Ascent / Bottom_Hold etc.) to the `phase` column. When automatic results are unclear, the step records segmentation failure points and uses manual intervention to confirm boundaries. Corresponds to dissertation §4.5. |
| Feature extraction | Computes spatial, temporal, and control domain quantitative metrics from normalized coordinates and exercise definition. When ⑥ Segmentation has populated the `phase` column, features in PHASE_AWARE_FEATURE_FAMILIES are also emitted at (rep_id, phase) granularity alongside rep-level records. |
